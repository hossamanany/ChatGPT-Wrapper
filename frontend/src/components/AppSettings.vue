...
const formSchema = object({
  ...
  openaiTemp: any()
    ...
    .refine((val) => validator.isFloat(val)),
  openaiModel: string(),
  openaiMaxTokens: any()
    .transform((val) => val.toString())
    .refine((val) => validator.isFloat(val)),
...
<template>
  <fwb-modal
    v-if="settingsStore.areSettingsVisible"
    @close="settingsStore.hideSettings()"
    not-escapable
    persistent
  >
    <template #header>
    ...
    <template #footer>
      <div class="modal-footer">
        ...
        >
        <fwb-button
          @click="settingsStore.hideSettings()"
          color="alternative"
          class="modal-button"
          >Close</fwb-button
        >
